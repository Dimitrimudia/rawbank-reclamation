...
Backend Java Spring Boot exposant `POST /api/complaints` (soumission vers Power Automate + indexation Elasticsearch optionnelle) et `GET /api/accounts` (liste des comptes via API protégée par JWT).
...
Routes:
- `POST /api/complaints` — soumet vers Power Automate; indexe ensuite dans Elasticsearch (si configuré). Réponse: `{ ok: true, powerAutomate: {...}, elasticsearch?: {...} }` ou `{ ok: false, error }`.
